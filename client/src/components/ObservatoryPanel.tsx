...
import { downloadMarkdown, observatoryMessagesToMarkdown, openPrintPdfPreview } from "@/lib/observatoryExport";
  ...
  return <section className="panel active observatory-panel">
    ...
    <div className="observatory-grid">
      ...
      <section className="observatory-section observatory-macro-section">
        ...
        <div className="observatory-chart-range" role="group" aria-label="總經圖表時間區間">{CHART_RANGES.map(item => <button key={item.value} type="button" className={chartRange === item.value ? "selected" : ""} onClick={() => setChartRange(item.value)}>{item.label}</button>)}</div><div className="observatory-alert-shortcuts" role="group" aria-label="警示門檻快捷設定"><span>警示快捷門檻</span>{ALERT_PRESETS.map(preset => <button key={preset.label} type="button" onClick={() => applyAlertPreset(preset.marketThreshold, preset.macroThreshold)}>{preset.label}（市場 {preset.marketThreshold}%／總經 {preset.macroThreshold}%）</button>)}</div>
