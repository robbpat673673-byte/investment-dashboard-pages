...
export function ManualRefreshPanel({ onCompleted }: { onCompleted: () => void }) {
  ...
  return <section className="manual-refresh-panel" aria-label="手動刷新管理介面">
    <div className="manual-refresh-head"><div><strong>資料刷新管理</strong><small>每日台北時間 08:00 自動更新 · 伺服器 SSE 即時回報</small></div><button type="button" className="manual-refresh-button" disabled={isStreaming} onClick={startRefresh}>{isStreaming ? "刷新執行中…" : "立即執行一次"}</button></div>
