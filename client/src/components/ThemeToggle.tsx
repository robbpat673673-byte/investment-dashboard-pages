...
export function ThemeToggle() {
  ...
  return <button type="button" className="theme-toggle" onClick={toggleTheme} aria-pressed={isDark} aria-label={isDark ? "切換為淺色模式" : "切換為深色模式"} title={isDark ? "切換為淺色模式" : "切換為深色模式"}>{isDark ? <Sun size={14} aria-hidden="true" /> : <Moon size={14} aria-hidden="true" />}<span>{isDark ? "淺色" : "深色"}</span></button>;
}
