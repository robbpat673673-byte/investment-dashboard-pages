...
import { buildNewsHealthTrend, type NewsHealthTrendPoint } from "@/lib/newsHealthTrend";
...
function FundCard({ fund, isFavorite = false, onToggleFavorite }: { fund: FundCardData; isFavorite?: boolean; onToggleFavorite?: (id: number) => void }) {
  ...
  return (
    <article className="fund-card">
      ...
      <div className="fc-ts">資料截至：{formatDate(fund.asOfDate)} · {fund.fundType === "domestic" ? "淨值" : "境外基金淨值"}</div>
...
function NewsSourceHealthTrend({ history }: { history: NewsHealthHistoryPoint[] }) {
  ...
  return <section className="news-health-trend" style={{ overflowX: "hidden" }} aria-label="新聞來源歷史趨勢"><div className="detail-section-title"><span>歷史健康趨勢</span><small>最近 {trend.batchCount} 次刷新；成功率以 fresh 次數累計計算</small></div><div className="news-health-chart-block" style={{ overflowX: "hidden" }}><strong>各來源累積抓取成功率</strong><ResponsiveContainer width="100%" height={230}><LineChart data={trend.successData} margin={{ top: 8, right: 18, left: 0, bottom: 8 }}><CartesianGrid strokeDasharray="3 3" stroke="var(--border)" /><XAxis dataKey="label" tick={{ fontSize: 10 }} minTickGap={28} /><YAxis domain={[0, 100]} tickFormatter={value => `${value}%`} tick={{ fontSize: 10 }} /><Tooltip formatter={(value: number) => `${value}%`} /><Legend wrapperStyle={{ fontSize: 11 }} />{trend.sources.map((source, index) => <Line key={source} type="monotone" dataKey={source} stroke={colors[index % colors.length]} strokeWidth={2} dot={false} connectNulls />)}</LineChart></ResponsiveContainer></div><div className="news-health-chart-block" style={{ overflowX: "hidden" }}><strong>各來源抓取延遲</strong><ResponsiveContainer width="100%" height={230}><LineChart data={trend.latencyData} margin={{ top: 8, right: 18, left: 0, bottom: 8 }}><CartesianGrid strokeDasharray="3 3" stroke="var(--border)" /><XAxis dataKey="label" tick={{ fontSize: 10 }} minTickGap={28} /><YAxis tickFormatter={value => `${value}ms`} tick={{ fontSize: 10 }} /><Tooltip formatter={(value: number) => `${value} ms`} /><Legend wrapperStyle={{ fontSize: 11 }} />{trend.sources.map((source, index) => <Line key={source} type="monotone" dataKey={source} stroke={colors[index % colors.length]} strokeWidth={2} dot={false} connectNulls />)}</LineChart></ResponsiveContainer></div></section>;
}
