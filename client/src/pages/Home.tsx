...
export default function Home() {
  ...
  const requestNewsSummary = (item: { id: number; title: string; summary: string | null; source: string; publishedAt: Date | string | null }) => { const key = String(item.id); setNewsSummaryLoadingId(key); setNewsSummaryErrorById(current => { const next = { ...current }; delete next[key]; return next; }); newsSummary.mutate({ id: String(item.id), title: item.title, summary: item.summary ?? "", source: item.source, publishedAt: item.publishedAt ? new Date(item.publishedAt).toISOString() : null }); };
