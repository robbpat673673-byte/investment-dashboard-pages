...
import { HISTORY_RANGE_KEY, historyRangeLabels, historyRanges, parseHistoryRange, sliceHistoryByRange, type HistoryRange } from "@/lib/historyRange";
...
function MarketSparkline({ history, percentChange, range }: { history: Array<{ date: string; value: number }>; percentChange: number | null; range: HistoryRange }) {
  ...
  return <div className="market-sparkline-wrap" aria-label={`${formatChartDate(history[0]?.date)} 至 ${formatChartDate(history.at(-1)?.date)} 的近期價格走勢`}><div className="sparkline-head"><span>近{historyRangeLabels[range]}價格走勢</span><span>{formatChartDate(history.at(-1)?.date)}</span></div><svg className={`sparkline ${valueClass(percentChange)}`} viewBox="0 0 100 100" preserveAspectRatio="none" role="img"><polyline points={points} className="sparkline-line" vectorEffect="non-scaling-stroke" /><circle cx="100" cy={points.split(" ").at(-1)?.split(",")[1] ?? "50"} r="2.3" className="sparkline-dot" vectorEffect="non-scaling-stroke" /></svg></div>;
}
...
function MarketCards({ market, now, range, onRangeChange, cardOrder, onCardOrderChange }: { market: MarketItem[]; now: Date; range: HistoryRange; onRangeChange: (value: HistoryRange) => void; cardOrder: string[]; onCardOrderChange: (nextOrder: string[]) => void }) {
  ...
  return (
    <div className="market-card-zone">
      <div className="market-card-controls"><span>按住卡片拖曳即可排序；此瀏覽器會記住你的順序。</span><HistoryRangeSwitcher value={range} onChange={onRangeChange} /><button type="button" className="sort-reset" onClick={() => onCardOrderChange([])}>重設排序</button></div>
