...
export default function Home() {
  ...
  const filteredNews = useMemo(() => (data?.news ?? []).filter(item => { const keyword = newsKeyword.trim().toLocaleLowerCase(); const matchesKeyword = !keyword || `${item.title} ${item.summary ?? ""} ${item.source}`.toLocaleLowerCase().includes(keyword); return matchesKeyword && (newsSource === "all" || item.source === newsSource) && (newsCategoryFilter === "all" || newsCategory(item.source, item.title) === newsCategoryFilter); }).sort((left, right) => new Date(right.publishedAt ?? 0).getTime() - new Date(left.publishedAt ?? 0).getTime()), [data?.news, newsSource, newsCategoryFilter, newsKeyword]);
