...
import { HISTORY_RANGE_KEY, MARKET_HISTORY_RANGE_KEY, historyRangeLabels, historyRanges, marketHistoryRangeLabels, marketHistoryRanges, parseHistoryRange, parseMarketHistoryRange, sliceHistoryByRange, sliceMarketHistoryByRange, type HistoryRange, type MarketHistoryRange } from "@/lib/historyRange";
...
function MarketRangeSwitcher({ value, onChange }: { value: MarketHistoryRange; onChange: (value: MarketHistoryRange) => void }) {
  return <div className="history-range-switcher market-range-switcher" aria-label="選擇市場走勢圖時間區間">{marketHistoryRanges.map(range => <button type="button" className={value === range ? "active" : ""} key={range} onClick={() => onChange(range)}>{marketHistoryRangeLabels[range]}</button>)}</div>;
}

function Sparkline({ history, annualReturn, range }: { history: FundCardData["history"]; annualReturn: number | null; range: HistoryRange }) {
  if (history.length < 2) return <div className="sparkline-empty">一年期淨值資料不足</div>;
  const values = history.map(point => point.nav);
  const minimum = Math.min(...values);
  const maximum = Math.max(...values);
  const numericRange = maximum - minimum || 1;
  const points = history.map((point, index) => {
    const x = (index / (history.length - 1)) * 100;
    const y = 92 - ((point.nav - minimum) / numericRange) * 78;
  ...
  return (
    <div className="sparkline-wrap" aria-label={`${formatChartDate(history[0]?.date)} 至 ${formatChartDate(history.at(-1)?.date)} 的一年期淨值走勢`}>
      <div className="sparkline-head"><span>近{historyRangeLabels[range]}淨值走勢</span><span>{formatChartDate(history[0]?.date)} — {formatChartDate(history.at(-1)?.date)}</span></div>
...
function MarketSparkline({ history, percentChange, range, onExpand, expanded = false }: { history: Array<{ date: string; value: number }>; percentChange: number | null; range: MarketHistoryRange; onExpand?: () => void; expanded?: boolean }) {
  if (history.length < 2) return <div className="market-sparkline-empty">近期價格資料不足</div>;
  const values = history.map(point => point.value);
  const minimum = Math.min(...values);
  const maximum = Math.max(...values);
  const numericRange = maximum - minimum || 1;
  const points = history.map((point, index) => `${((index / (history.length - 1)) * 100).toFixed(2)},${(92 - ((point.value - minimum) / numericRange) * 78).toFixed(2)}`).join(" ");
  const content = <><div className="sparkline-head"><span>近{marketHistoryRangeLabels[range]}價格走勢</span><span>{formatChartDate(history.at(-1)?.date)}{onExpand ? " · 點擊放大" : ""}</span></div><svg className={`sparkline ${valueClass(percentChange)}`} viewBox="0 0 100 100" preserveAspectRatio="none" role="img"><polyline points={points} className="sparkline-line" vectorEffect="non-scaling-stroke" /><circle cx="100" cy={points.split(" ").at(-1)?.split(",")[1] ?? "50"} r="2.3" className="sparkline-dot" vectorEffect="non-scaling-stroke" /></svg></>;
  const label = `${formatChartDate(history[0]?.date)} 至 ${formatChartDate(history.at(-1)?.date)} 的近${marketHistoryRangeLabels[range]}價格走勢`;
  if (onExpand) return <button type="button" className="market-sparkline-wrap interactive" aria-label={`放大 ${label}`} onClick={onExpand}>{content}</button>;
  return <div className={`market-sparkline-wrap${expanded ? " expanded" : ""}`} aria-label={label}>{content}</div>;
}

function FundCard({ fund, now, range, isFavorite = false, onToggleFavorite }: { fund: FundCardData; now: Date; range: HistoryRange; isFavorite?: boolean; onToggleFavorite?: (id: number) => void }) {
...
function FundBrowsePanel({ funds, favoriteFunds, now, range, onRangeChange, query, scope, currency, sortKey, currencies, importMessage, onQueryChange, onScopeChange, onCurrencyChange, onSortChange, onToggleFavorite, onClearFavorites, onExportFavorites, onImportFavorites }: {
  funds: FundCardData[];
  ...
  return <>
    <div className="sec-title">{scopeLabel} — 搜尋與篩選 <HistoryRangeSwitcher value={range} onChange={onRangeChange} /></div>
    ...
    {funds.length === 0 ? <div className="empty-inline">沒有符合條件的基金，請調整搜尋字詞或篩選條件。</div> : <div className="fund-grid">{funds.map(fund => <FundCard key={fund.id} fund={fund} now={now} range={range} isFavorite={favoriteFunds.some(item => item.id === fund.id)} onToggleFavorite={onToggleFavorite} />)}</div>}
  </>;
}
...
function MarketCards({ market, now, range, onRangeChange, cardOrder, onCardOrderChange }: { market: MarketItem[]; now: Date; range: MarketHistoryRange; onRangeChange: (value: MarketHistoryRange) => void; cardOrder: string[]; onCardOrderChange: (nextOrder: string[]) => void }) {
  const [draggedTicker, setDraggedTicker] = useState<string | null>(null);
  const [expandedMarket, setExpandedMarket] = useState<MarketItem | null>(null);
  ...
  return (
    <div className="market-card-zone">
      <div className="market-card-controls"><span>按住卡片拖曳即可排序；圖表可點擊放大。</span><MarketRangeSwitcher value={range} onChange={onRangeChange} /><button type="button" className="sort-reset" onClick={() => onCardOrderChange([])}>重設排序</button></div>
      <div className="index-bar" aria-label="可拖曳排序的市場行情卡片">
        {cards.map(item => {
          const freshness = classifyDataFreshness(item.quoteDate, now, "market");
          const session = getMarketSession(item.ticker, now);
          return <article className={`idx-card ${draggedTicker === item.ticker ? "dragging" : ""}`} key={item.ticker} draggable onDragStart={() => setDraggedTicker(item.ticker)} onDragOver={event => event.preventDefault()} onDrop={() => reorder(item.ticker)} onDragEnd={() => setDraggedTicker(null)}>
            <span className="idx-drag-handle" aria-hidden="true">⋮⋮</span><div className="idx-name">{item.name}</div><div className={`idx-val ${valueClass(item.percentChange)}`}>{formatNumber(item.price)}</div><div className={`idx-chg ${valueClass(item.percentChange)}`}>{item.change === null ? "--" : `${item.change > 0 ? "+" : ""}${formatNumber(item.change)}`} ({returnText(item.percentChange)})</div><MarketSparkline history={sliceMarketHistoryByRange(item.history ?? [], range)} percentChange={item.percentChange} range={range} onExpand={() => setExpandedMarket(item)} /><div className={`market-session ${session.state}`}><strong>{session.stateLabel}</strong><span>{session.timezoneLabel} {session.localTime}</span><small>標準時段 {session.scheduleLabel}</small><em>{session.detail}</em></div><div className="idx-ts">資料截至：{formatQuoteDate(item.quoteDate, now)} · {item.quoteStatus} · <span className={`data-freshness ${freshness.kind}`}>{freshness.label}</span><small>{freshness.detail}</small></div>
          </article>;
        })}
      </div>
      <Dialog open={Boolean(expandedMarket)} onOpenChange={open => { if (!open) setExpandedMarket(null); }}><DialogContent className="market-chart-dialog"><DialogHeader><DialogTitle>{expandedMarket?.name ?? "市場走勢"}</DialogTitle><DialogDescription>{expandedMarket ? `${expandedMarket.ticker} · 近${marketHistoryRangeLabels[range]}價格走勢；資料截至 ${formatQuoteDate(expandedMarket.quoteDate, now)}` : ""}</DialogDescription></DialogHeader>{expandedMarket ? <MarketSparkline history={sliceMarketHistoryByRange(expandedMarket.history ?? [], range)} percentChange={expandedMarket.percentChange} range={range} expanded /> : null}</DialogContent></Dialog>
    </div>
  );
}
...
export default function Home() {
  ...
  const [historyRange, setHistoryRange] = useState<HistoryRange>(() => parseHistoryRange(typeof window === "undefined" ? null : window.localStorage.getItem(HISTORY_RANGE_KEY)));
  const [marketHistoryRange, setMarketHistoryRange] = useState<MarketHistoryRange>(() => parseMarketHistoryRange(typeof window === "undefined" ? null : window.localStorage.getItem(MARKET_HISTORY_RANGE_KEY)));
  ...
  useEffect(() => { window.localStorage.setItem(HISTORY_RANGE_KEY, historyRange); }, [historyRange]);
  useEffect(() => { window.localStorage.setItem(MARKET_HISTORY_RANGE_KEY, marketHistoryRange); }, [marketHistoryRange]);
