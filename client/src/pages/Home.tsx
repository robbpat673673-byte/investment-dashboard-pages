...
export default function Home() {
  ...
  const filteredNews = useMemo(() => (data?.news ?? []).filter(item => (newsSource === "all" || item.source === newsSource) && (newsCategoryFilter === "all" || newsCategory(item.source, item.title) === newsCategoryFilter)).sort((left, right) => new Date(right.publishedAt ?? 0).getTime() - new Date(left.publishedAt ?? 0).getTime()), [data?.news, newsSource, newsCategoryFilter]);
  ...
  return <div className="shell">
    ...
    <main className="main">
      ...
      {activeTab === "news" && <section className="panel active"><div className="sec-title">財經即時新聞 <small>多來源標題與公開摘要</small></div><p className="news-disclosure">整合華爾街日報、CNBC、MarketWatch、Financial Times 與 Google 新聞 RSS；付費或需登入的內容僅提供合法取得的標題、摘要、時間與原文連結。</p><div className="news-filter-bar" aria-label="財經新聞來源與分類篩選"><label>來源<select value={newsSource} onChange={event => setNewsSource(event.target.value)} aria-label="篩選財經新聞來源"><option value="all">全部來源</option>{newsSources.map(source => <option value={source} key={source}>{source}</option>)}</select></label><div className="news-category-chips" aria-label="新聞分類" role="group">{["all", "市場", "總經", "基金／債券", "台灣"].map(category => <button type="button" className={`filter-chip ${newsCategoryFilter === category ? "active" : ""}`} key={category} onClick={() => setNewsCategoryFilter(category)}>{category === "all" ? "全部分類" : category}</button>)}</div><span>目前 {filteredNews.length} 則</span></div>{newsHighlights.length > 0 ? <section className="news-highlights" aria-label="快速掌握新聞重點"><div className="detail-section-title"><span>快速掌握</span><small>優先顯示最新三則</small></div>{newsHighlights.map(item => <a className="news-highlight" href={item.url} target="_blank" rel="noreferrer" key={`highlight-${item.id}`}><strong>{item.title}</strong><span>{item.source} · {formatDateTime(item.publishedAt)}</span></a>)}</section> : null}<div className="news-list">{filteredNews.length === 0 ? <div className="empty-inline">等待每日 RSS 更新或請調整來源篩選。</div> : filteredNews.map(item => <article className="news-item" key={item.id}><a className="news-title" href={item.url} target="_blank" rel="noreferrer">{item.title}</a>{item.summary ? <p className="news-body">{item.summary}</p> : null}<div className="news-meta"><span>{formatDateTime(item.publishedAt)}</span><span>{item.source}</span></div></article>)}</div></section>}
